...
const updateVibePlaylist = new ExecutionDelay(
  ...
  { delay: 7000, isThrottle: true, leading: true },
).start;
...
const ExtractedData = {
  ...
  async loadEntities(entities) {
    ...
    return this.queueController.entityLoader.entityProvider.loadEntities(
      entities,
    );
  },
  ...
  async populate(fromIndex, after, before, ordered) {
    return new Promise((resolve, reject) => {
      if (typeof fromIndex !== "number") {
        throw new TypeError(
          `'fromIndex' must be a number, but received type '${typeof fromIndex}'`,
        );
      }
      if (typeof after !== "undefined" && typeof after !== "number") {
        throw new TypeError(
          `'after' must be a number, but received type '${typeof after}'`,
        );
      }
      if (typeof before !== "undefined" && typeof before !== "number") {
        throw new TypeError(
          `'before' must be a number, but received type '${typeof before}'`,
        );
      }
      ...
      const unloadedAfterTracks = Tracks.getUnloadedTracks(
        fromIndex,
        after,
        "down",
      );
      const unloadedBeforeTracks = Tracks.getUnloadedTracks(
        fromIndex,
        before,
        "up",
      );
      const unloaded = new Map(
        [...unloadedAfterTracks].concat([...unloadedBeforeTracks]),
      );
      ...
      ExtractedData.loadEntities(unloadedTracks)
        .then((tracks) => {
...
const Vibe = {
  ...
  async startFromCurrentTrack() {
    const trackId = Tracks.currentId;
    if (!trackId) {
      throw new Error("Current track id is not available");
    }
    ...
    const context = await Controller.contextController.factory.createContext({
      data: {
        ...
        meta: { id: seed },
        seeds: [seed],
        from: `web-radio-track-${trackId}-default`,
        includeTracksInResponse: true,
...
const Toggles = {
  ...
  /** @returns {void} */
  setPosition(value) {
    unblockedController.setProgress(value);
  },
  /** @returns {void} */
  setSpeed(value) {
    unblockedController.setSpeed(value);
  },
  /** @returns {void} */
  setVolume(value) {
    unblockedController.setVolume(value);
  },
  _likeDislikeData: {
    get albumId() {
  ...
  /** @returns {void} */
  toggleTrackLike() {
    if (!Toggles._likeDislikeData.userId)
  ...
  toggleTrackDisike: (function () {
    const nextTrack = new ExecutionDelay(
      () => {
        Toggles.next();
      },
      { delay: 300 },
  ...
  async play(index) {
    return new Promise((resolve, reject) => {
      ...
      result = unblockedController.playContext(
        Toggles.createPlayContext(index),
      );
      Toggles._rejectPlay(reject, result);
    });
  },
  _rejectPlay(reject, result) {
    result?.catch(reject);
  },
...
const { Controller: unblockedController, Toggles: unblockedToggles } =
